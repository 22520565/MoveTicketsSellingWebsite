...
import AdditionalItemCard from "../Components/AdditionalItemCard";
...
export default function OffLineTicketBooking() {
  ...
  const filteredFilmShow = filmShowList.filter(
    (element) => element.filmName === filmName
    //date
  );
  ...
  const fetchRoom = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/rooms/676b6c0db2621ba199d649a1"
      );
  ...
  return (
    <>
      {currentPage === 1 ? (
        <div className="OfflineTicketPage bg-black">
                ...
                <div className="row" key={element._id}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (updatedQuantity < 0) updatedQuantity = 0; // Parse the new quantity
                  ...
                  return (
                    <AdditionalItemCard
                      ...
                      onChangeQuantity={(newQuantity) => {
                        ...
                        if (newQuantity > 4) {
                          alert(
                            "Bạn chỉ có thể mua tối đa 4 sản phẩm loại này"
                          );
                          return;
                        }
  ...
  return (
    <div className="BottomBar">
      ...
      <div>
        ...
        {currentPage === 1 ? (
          <button onClick={onPage1Submit}>Tiếp tục</button>
        ) : (
          <div style={{ display: "flex", flexDirection: "row", gap: "10px" }}>
            <button onClick={onReturnToPage1}>Trở lại</button>
            <button onClick={onPage2Submit}>Xác nhận</button>
          </div>
        )}
      </div>
    </div>
